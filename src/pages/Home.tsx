...
type FilterType = 'all' | 'action-needed' | 'confirmed' | 'waiting' | 'draft' | 'blocked';
type RequestStatus = 'confirmed' | 'waiting' | 'blocked' | 'draft';
...
const REQUESTS: Request[] = [
  {
    ...
    image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuC-dv9qAKQ5SRv2Ebwj3pXLCOFw3TJSh0G334exiq1WfkRVEHxihbS3hfoeB-eI7Eajrb-V0OhcBreHZDNFSnRYi-Q4ZMPpp4upaFDNI2tdWv1QLqGWzTe89C0A9R9wkvdBpOjGnroo0Wev0atQDzGK3AOVkT8Z0D6JCSXc_DHNZmKf2HEnT9-2XamilPEjfchpo4C6QngmKnyGWgC4ATVauciufY2MsYynP1bcrd2nBIbz640C61WUg4XSmM_u0iMlgNAKQNLC2g',
    buttonText: 'Details →'
  },
  {
    id: '2',
...
export default function Home() {
  ...
  const getFilteredRequests = (): Request[] => {
    ...
    if (filter === 'action-needed') return REQUESTS.filter(r => r.status === 'waiting' || r.status === 'blocked');
    return REQUESTS.filter(r => r.status === filter);
  };

  const getFilterCount = (filterType: FilterType): number => {
    if (filterType === 'all') return REQUESTS.length;
    if (filterType === 'action-needed') return REQUESTS.filter(r => r.status === 'waiting' || r.status === 'blocked').length;
    return REQUESTS.filter(r => r.status === filterType).length;
  };

  const filteredRequests = getFilteredRequests();
  const filters: { id: FilterType; label: string; showCount?: boolean }[] = [
    { id: 'all', label: 'All' },
    { id: 'action-needed', label: 'Action Needed', showCount: true },
    ...
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden pb-32 graph-paper-bg font-sans text-ink-black">
        ...
        <div className="hide-scrollbar flex gap-4 overflow-x-auto px-6 py-2">
          {filters.map((f) => (
            <button
              ...
              className={`sketch-border offset-shadow-sm flex h-10 shrink-0 items-center justify-center px-5 active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all ${
                filter === f.id
                  ? 'bg-accent text-white'
                  : 'bg-white text-ink-black'
              }`}
            >
              <span className="font-hand text-lg font-bold">{f.label}</span>
              {f.showCount && (
                <span className={`ml-2 font-hand text-lg ${filter === f.id ? 'text-white' : 'text-ink-red'}`}>
                  {' '}({getFilterCount(f.id)})
                </span>
              )}
            </button>
      ...
      <div className="px-6 pt-4 pb-2">
        <div className="flex items-baseline justify-between">
          ...
          <span className="font-hand text-lg text-slate-500">{filteredRequests.length} total</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-8 px-6 pb-6 mt-4">
        {filteredRequests.map((request) => (
          request.status === 'confirmed' && request.image ? (
            <div
              key={request.id}
              className="sketch-border offset-shadow bg-white overflow-hidden flex flex-col group transition-transform active:translate-x-1 active:translate-y-1 active:shadow-none"
            >
              ...
              <div className="p-5">
                <h3 className="font-hand text-3xl font-bold leading-none mb-2">{request.title}</h3>
                {request.date && (
                  <div className="flex items-center gap-2 text-slate-600 mb-4">
                    <span className="material-symbols-outlined text-lg">calendar_month</span>
                    <span className="font-hand text-lg">{request.date}</span>
                  </div>
                )}
          ...
          ) : request.status === 'waiting' && request.image ? (
            <Link
              key={request.id}
              to={request.link || '#'}
              className="sketch-border offset-shadow bg-white overflow-hidden flex flex-col active:translate-x-1 active:translate-y-1 active:shadow-none transition-all"
            >
              <div className="h-48 relative border-b-2 border-ink-black">
                <img
                  alt={request.title}
              ...
              <div className="p-5">
                <h3 className="font-hand text-3xl font-bold leading-none mb-1">{request.title}</h3>
                {request.description && (
                  <p className="font-hand text-lg text-ink-amber mb-3">{request.description}</p>
                )}
                <div className="flex items-center justify-between">
                  <div className="flex -space-x-2">
            ...
            <Link
              key={request.id}
              to={request.link || '#'}
              className="sketch-border offset-shadow bg-white p-5 flex gap-4 active:translate-x-1 active:translate-y-1 active:shadow-none transition-all"
            >
              ...
              <div className="flex-1 flex flex-col justify-center">
                ...
                <h3 className="font-hand text-2xl font-bold">{request.title}</h3>
                <div className="flex items-center justify-between">
                  <p className="font-hand text-lg text-slate-500">{request.date}</p>
            ...
            >
              ...
              <div className="flex-1 flex flex-col justify-center">
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <button className="font-hand text-lg font-bold underline decoration-2">{request.buttonText}</button>
                </div>
              </div>
            </Link>
          ) : null
