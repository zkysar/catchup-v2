...
- Per-event data source controls beyond the per-plan consent prompt with sharing granularity (users connect their calendar once in account settings, choose free/busy-only or event-name sharing on each plan, but cannot select which individual calendars or sub-calendars to share).
...
The AI chat agent is the primary interface for organizers and is contextual to the current plan. This section defines how the agent behaves across plan states, what types of responses it produces, and how it handles errors and ambiguity.

#### Agent Availability & Scope

- The AI chat agent appears as a floating panel on all plan pages for the **organizer only**. Invitees do not interact with the AI chat; their interaction is through the availability form, time-slot grid, and location suggestion fields.
...
- The agent accepts three input modes: **text** (typed), **voice** (transcribed via speech-to-text), and **image** (processed via OCR/vision). All three modes feed into the same processing pipeline; the agent does not behave differently based on input mode.
...
- **Collecting Availability (few responses):** "[N] of your invitees have responded so far. I'll suggest the best times once more responses come in."
- **Collecting Availability (enough responses for suggestions):** "[N] invitees have responded. Saturday 3-6pm works for the most people. Want me to show you the options?"
...
- The agent asks **one targeted question at a time**, never a list of multiple questions. This keeps the interaction conversational and avoids overwhelming the organizer.
...
- Each option shows: time slot, number of attendees, and who is missing.
- The top option has a suggestion chip. Others are selectable as alternatives.
- Example:
  - "Saturday 8:30pm: 5 of 8 (missing: Jordan, Sophie B., Taylor)" [Confirm this time]
  - "Saturday 7pm: 3 of 8"
  - "Friday 7pm: 2 of 8"

**7. Conversational Fallback**
Used when the organizer's input doesn't map to a plan action.
...
**Voice Input:**
1. Organizer taps the microphone icon and speaks.
2. Speech-to-text transcribes the input in real-time (streaming transcription).
3. The transcription is displayed as editable text before the agent processes it. Organizer can correct before sending.
...
**Image Input:**
1. Organizer taps the image icon and selects or captures a photo.
...
- **Action-oriented.** Every agent response should either (a) present extracted data, (b) ask one clarifying question, (c) offer an actionable suggestion, or (d) report plan status. The agent avoids generic commentary, filler phrases, or restating what the organizer already said.
...
- **AI response latency (>5 seconds):** A loading indicator is shown with a contextual message (e.g., "Finding the best times..." or "Processing your image..."). If latency exceeds 15 seconds, the agent shows: "This is taking longer than usual. You can continue filling in details manually while I work on it."
...
- **Rate limiting:** If the organizer sends many rapid messages (e.g., during a frustrating interaction), the agent processes them sequentially without dropping messages. No user-facing rate limit errors are shown for organizer interactions.
...
- **AI Chat Agent:** Floating AI chat available on all plan pages for the organizer. Accepts text, voice, and image input. See Flow 9 (AI Chat Agent Interaction Model) for detailed conversational design, response types, voice/image handling, tone guidelines, and error/degradation states.
- **Proactive Suggestions:** AI surfaces time, venue, and next step suggestions as actionable "chips" or "pills" at appropriate moments (especially when conflicts arise or when enough responses are collected). At most 3 chips are shown simultaneously to avoid decision overload.
- **Calendar Conflicts:** Surface scheduling conflicts detected from connected calendars (e.g., an invitee has a conflicting event) and suggest alternative times. Conflict details respect the invitee's sharing granularity preference — event names are only shown if the invitee opted into "Share event names."
- **Response Conflicts:** Identify mismatches across invitee-submitted availability and suggest times that maximize attendance.
- **Smart Defaults:** When parsing ambiguous input (e.g., "morning"), AI uses generous but reasonable defaults and presents them for confirmation. Low-confidence extractions are visually distinguished (e.g., dotted underline).
...
- **Graceful Degradation:** If the AI service is unavailable, the plan page falls back to a fully manual form-based flow. All core functionality (plan creation, availability collection, confirmation) works without the AI.
...
- **Calendar Sync (Optional):** Invitee/organizer can (optionally) connect Google Calendar to check availability, but not required for MVP flow to proceed. Calendar data is never cached server-side; it is fetched on-demand for the plan's date range and discarded after availability is computed. Fellow invitees cannot see each other's availability data, and anonymous users see no calendar-derived data from other participants. The calendar owner always sees their own event names during the availability review to help them decide. Each time calendar data would be shared with a plan, the user is shown a per-plan consent prompt ("Share your availability with [Organizer Name] for this plan?"), even if their calendar is already connected.
- **Calendar Sharing Granularity:** During the per-plan consent prompt, invitees choose between two sharing modes: (a) **Free/Busy only** (default) — only free/busy time blocks are shared with the organizer, with no event labels; (b) **Share event names** — free/busy blocks are shared along with event titles, helping the organizer understand the nature of conflicts. In both modes, event descriptions, attendee lists, and other metadata are never exposed. The invitee can change this setting at any time before the plan is confirmed. The AI chat agent respects the invitee's sharing preference — it can reference event names in its suggestions only when the invitee has opted into "Share event names."
...
- **Calendar Data Privacy:** Calendar data is fetched on-demand and never cached server-side. By default, only free/busy blocks are shared with the organizer and the AI. If the invitee explicitly opts into "Share event names," event titles are also visible to the organizer and the AI, but event descriptions, attendee lists, and other metadata are never exposed regardless of the sharing mode. Invitees cannot see each other's availability data; only the organizer sees the aggregated availability grid. Anonymous users see no calendar-derived data from other participants. Per-plan consent (with sharing granularity selection) is required before sharing calendar availability with an organizer.
...
- Google Calendar integration (optional, OAuth with minimal scope — `calendar.freebusy` for free/busy-only sharing, or `calendar.readonly` when the invitee opts into "Share event names"; never write access; per-plan consent with sharing granularity selection required; no server-side caching of calendar data)
